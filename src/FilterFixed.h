 ...
 * \f[
 * H(z) = 2^{-a}
 * \frac{1 + 2z^{-1} + 1z^{-2}}{1 + (-2 + 2^{-b})z^{-1} + (1-2^{-b} + 4* 2^{-a})z^{-2}}
 * \f]
 *
 ...
 *
 * 	Here are the specifications for a single stage filter, for values \f$a=2b+4\f$
...
class FixedFilter{
	public:
		...
		uint8_t a; //!< Filter `a` coefficient
		uint8_t b; //!< Filter `b` coefficient
