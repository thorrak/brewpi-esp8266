...
int WiFiServerFixed::setTimeout(uint32_t seconds){
  struct timeval tv;
  tv.tv_sec = seconds;
  tv.tv_usec = 0;
...
size_t WiFiServerFixed::write(const uint8_t *data, size_t len){
  return 0;
}

void WiFiServerFixed::stopAll(){}

WiFiClientFixed WiFiServerFixed::available(){
  if(!_listening)
    return WiFiClientFixed();
  int client_sock;
  ...
  if(client_sock >= 0){
    ...
    if(setsockopt(client_sock, SOL_SOCKET, SO_KEEPALIVE, (char*)&val, sizeof(int)) == ESP_OK) {
      ...
      if(setsockopt(client_sock, IPPROTO_TCP, TCP_NODELAY, (char*)&val, sizeof(int)) == ESP_OK)
        return WiFiClientFixed(client_sock);
    }
  }
  return WiFiClientFixed();
}

void WiFiServerFixed::begin(uint16_t port){
    begin(port, 1);
}

void WiFiServerFixed::begin(uint16_t port, int enable){
  if(_listening)
    return;
  if(port){
      _port = port;
...
void WiFiServerFixed::setNoDelay(bool nodelay) {
    _noDelay = nodelay;
}

bool WiFiServerFixed::getNoDelay() {
    return _noDelay;
}

bool WiFiServerFixed::hasClient() {
    if (_accepted_sockfd >= 0) {
      return true;
    }
    struct sockaddr_in _client;
...
void WiFiServerFixed::close(){
  end();
}

void WiFiServerFixed::stop(){
  end();
}
